...
if [[ ! -f "${project_root}/just_scripts/output/${tag}-${version}-${git_branch}.iso" ]]; then
    just build-iso "$image" "$version"
fi
...
${container_mgr} run --rm --cap-add NET_ADMIN \
    ...
    --env "BOOT_MODE=uefi" \
    --device=/dev/kvm \
    --volume "${workspace}/just_scripts/output/${tag}-${version}-${git_branch}.iso":/boot.iso \
